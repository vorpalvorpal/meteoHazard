# Tests for ventilation_state()'s core dispersion state: Pasquill-Turner
# stability (primary), the optional shear override, the calm-wind floor, and
# the mixing-depth fallbacks. The stability index s runs 0 (class A) to 5
# (class F). These fixtures never set temperature_2m/relative_humidity_2m, so
# the v3 nocturnal cold-pool cap (pool_cap, default TRUE) never engages here
# (have_pool_thermal = FALSE) -- h_mix stays the raw BLH/fallback tested below.
...
test_that("calm wind forces the s = 4.25 stability override", {
  expect_equal(ventilation_state(md(0.2, 0, 10))$s, 4.25)
  expect_true(ventilation_state(md(0.2, 0, 10))$is_calm)
})

test_that("NA wind is treated as calm (s = 4.25)", {
  expect_equal(ventilation_state(md(NA_real_, 0, 10))$s, 4.25)
})

# ── Effective wind floor ────────────────────────────────────────────────────
test_that("u_eff is floored at U_CALM_FLOOR and otherwise passes through", {
  expect_equal(ventilation_state(md(0.2, 0, 10))$u_eff, 0.5)
  expect_equal(ventilation_state(md(3.0, 0, 50))$u_eff, 3.0)
  expect_equal(ventilation_state(md(NA_real_, 0, 10))$u_eff, 0.5)
})

# ── Mixing-depth fallbacks ───────────────────────────────────────────────
# md() supplies no temperature_2m/relative_humidity_2m, so have_pool_thermal
# is FALSE and the v3 cold-pool cap never engages (see file header) -- h_mix
# below is always the raw BLH or its stability-aware fallback.
test_that("h_mix passes through when boundary_layer_height is present", {
  expect_equal(ventilation_state(md(4, 0, 50, boundary_layer_height = 750))$h_mix, 750)
})

test_that("NA boundary layer falls back by stability: 200 stable/calm, 600 otherwise", {
  expect_equal(ventilation_state(md(0.2, 0, 10))$h_mix, 200) # calm
  expect_equal(ventilation_state(md(1.5, 0, 10))$h_mix, 200) # stable night (s = 5)
  expect_equal(ventilation_state(md(1.0, 800, 0))$h_mix, 600) # unstable day (s = 0)
})
...
test_that("non-positive boundary layer is floored to the fallback (no zero h_mix)", {
  ...
  expect_equal(ventilation_state(md(1.5, 0, 10, boundary_layer_height = 0))$h_mix, 200)
  expect_equal(ventilation_state(md(1.0, 800, 0, boundary_layer_height = -50))$h_mix, 600)
})

# ── Shear override ──────────────────────────────────────────────────────────
test_that("shear override yields a stable class for strong shear", {
  st <- ventilation_state(md(1.0, 0, 50, wind_speed_80m = 4.0),
                          stability = "shear")
...
test_that("shear override falls back to s = 4.25 when 80 m wind is missing", {
  st <- ventilation_state(md(2.0, 0, 50, wind_speed_80m = NA_real_),
                          stability = "shear")
...
test_that("the helper is vectorised and returns aligned vectors", {
  st <- ventilation_state(md(c(1.0, 4.0, 0.2), c(800, 0, 0), c(0, 90, 10)))
