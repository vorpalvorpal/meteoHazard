...
# v3 design). Detection: the v3 `litter_hazard_vec()` introduces a roughness-
...
litter_v3_available <- function() {
  exists("litter_hazard_vec", mode = "function") &&
    "z0" %in% names(formals(litter_hazard_vec))
}
...
describe("litter_hazard_vec() [v3]", {
  ...
  describe("input validation", {
    ...
    it("rejects parameter sets that violate ordering constraints", {
      ...
      expect_error(litter_hazard_vec(12, 8, 0, 0.1, ustar_t0 = 1.2, ustar_ref = 1.0))
      expect_error(litter_hazard_vec(
...
describe("litter_hazard() [v3]", {
  ...
  it("forwards calibration parameters through to the vector API", {
    ...
    expect_equal(litter_hazard(met), 0)            # gated by default
